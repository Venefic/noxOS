...
.set MAGIC,     0x1BADB002          # 'magic number' lets find the header
...
# to be withing the first 8 KiB of the kernel file.
.section .multiboot
.align 4
.long MAGIC
.long FLAGS
...
# which means the kernel file is smaller because it does not contain an unitialized stack.
# The stack on x86 must be 16-bit aligned according to the System V ABI standard and de-facto extensions.
...
_start:
    ...
    # Interrupts are disabled. Paging is disable.
    # The processor is as defined in the multiboot standard.
    ...
    # no safeguards, no debugging mechanisms, only what the kernel provides itself.
    # It has absolute and complete power of the machine.

    # To set up a stack, we set the esp register to point to the top of our stack
    # (as it grows downwards on x86 systems).
    # This is necessarily done in assembly as languages such as C cannot function
    # without a stack.
    mov $stack_top, %esp

    # This is a good place to initialize crucial processor state before the
    # high-level kernel in entered.
    # It's best to minimize the early environment where crucial are offline.
    # Note that the processor is not fully initialized yet:
    # Features such as floating point instructions and instruction set extensions
    # are not initialized yet. The GDT should be loaded here. Paging should be enabled here.
    ...
    # The stack was originally 16-byte aligned above and we've since
    # pushed a multiple of 16 bytes to the stack since
    # (pushed 0 bytes so far) and the alignment is thus preserved
    # and the call is well defined
    call kernel_main

    # If the system has nothin more to do, put the computer into an
    # infinite loop. To do that:
    # 1) Disable interrupts with cli (clear interrupt enable in eflags).
    #    They are already disabled by the bootloader, so this is not needed.
    #    Mind that you might later enable interrupts and return from
    #    kernel_main (which is sort of nonsensical to do).
    # 2) Wait for the nex interrupt to arrive with hlt (halt instruction).
    #    Since they're disabled, this will lock up the computer.
...
# This is useful when debugging of when you implement call tracing
.size _start, . - _start
